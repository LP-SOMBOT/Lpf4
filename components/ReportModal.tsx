...
export const ReportModal: React.FC<Props> = ({ question, chapterId, onClose }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await set(reportRef, {
                ...
                category: category,
                reason: details, // Optional message
    ...
    return (
        <Modal isOpen={true} title="Report / Soo Sheeg" onClose={onClose}>
            <div className="space-y-4">
                <div className="bg-slate-100 dark:bg-slate-900 p-3 rounded-xl">
                    <p className="text-xs text-slate-500 font-bold uppercase mb-1">Question / Su'aasha</p>
                    <p className="text-sm font-bold text-slate-800 dark:text-white line-clamp-2">{question.question}</p>
                </div>

                <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Problem / Cilada</label>
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Details / Faahfaahin (Optional)</label>
                    <textarea 
                        value={details}
                        onChange={(e) => setDetails(e.target.value)}
                        placeholder="Explain the issue / Faahfaahi cilada..."
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <Button fullWidth onClick={handleSubmit} isLoading={loading}>Submit / Dir</Button>
                </div>
            </div>
        </Modal>
    );
